...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing when |a| <= 10.
    ...
    assert (lt(0.0, s) or eq(0.0, s)) and (lt(s, 1.0) or eq(s, 1.0))
    assert_close(1.0 - s, minitorch.operators.sigmoid(-a))
    assert_close(minitorch.operators.sigmoid(0.0), 0.5)
    if -10.0 < a < 10.0:
        assert minitorch.operators.sigmoid(a + 0.01) > s
        assert minitorch.operators.sigmoid(a - 0.01) < s
...
def test_other(a: float, b: float) -> None:
    ...
    # Test inv(x) that x != 0 and inv(inv(x)) = x
    if abs(a) > 1e-5:
        assert_close(inv(inv(a)), a)
    # Test log(exp(x)) = x
    if a < 20:
        assert_close(minitorch.operators.log(minitorch.operators.exp(a)), a)
    # Test exp(log(x)) = x
    if a > 0:
        assert_close(minitorch.operators.exp(minitorch.operators.log(a)), a)
    # Test neg(neg(x)) = x
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    sum3 = minitorch.operators.sum(addLists(ls1, ls2))
    assert_close(sum1 + sum2, sum3)
